...
\item {\color{red} Specify} a process that ensures a high quality {\color{red}{MPI Standard Document}} that is both well thought through{\color{green}, but also} allows for fixes {\color{green} to the MPI Standard Document for issues} found in final review stages.
...
into that Document, and {\color{blue} typically} spans two subsequent physical MPI forum
meetings:
...
\begin{enumerate}
\item Prior to {\color{green}four} weeks before the start of the RCM:
  \begin{itemize}
...
\item At least {\color{green}four} weeks before the start of the RCM:
  \begin{enumerate}
...
\item In the {\color{green}four}-week window before the start of the RCM:
  \begin{enumerate}
  ...
  \item Chapter Committees continue to work on still-unresolved
    ...
    Editor published the final draft at the {\color{green}four}-week window must be
...
\item At the RCM:
  ...
  \item Items that must be specifically itemized and discussed with
    ...
    \item Any changes that were made within {\color{green}four} weeks of the beginning
      of the RCM.
    \end{itemize}
...
\item At the FRM:
  ...
  \item On a different calendar date than ballots listed in the
    previous two bullets (but at the same FRM), the MPI Forum
    Secretary conducts a final ballot on the entire Document.\footnote{{\color{green}Historical note: prior MPI Standard Documents had ballots for
    each chapter and then the entire document.  This is no longer the
    case.}}
    \begin{itemize}
