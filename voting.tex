...
{\Huge What about making this section for {\em any} MPI Document --
  even companion documents?}
...
into that Document, and spans two subsequent physical MPI forum
meetings:

\begin{itemize}
\item Release Candidate Meeting (RCM): The first physical MPI Forum
  meeting in the sequence is called the Release Candidate Meeting.
\item Final Ratification Meeting (FRM): The second meeting in the
  sequence is called the Final Ratification Meeting.
\end{itemize}
...
\item At the FRM:
  ...
  \item The MPI Forum Secretary conducts a ballot for each individual
    ...
    Ballots that fail must either have their changes reverted or delay
  ...
  \item The MPI Forum Secretary conducts a series of ballots for all
    ...
    change must be either reverted or delay the ratification process.
  \item On a different calendar date than ballots listed in the
    previous two bullets (but at the same FRM), the MPI Forum
    Secretary conducts a final ballot on the entire Document.
    \begin{itemize}
    \item If the ballot passes, the MPI Standard Document Editor adds
      ...
      web site.
    \item If the ballot fails, the entire raditication process must be
      repeated.
    \end{itemize}
  \item Historical note: prior Documents had ballots for each
    chapter and then the entire document.  This is no longer the
    case.
  \end{itemize}
\end{enumerate}
